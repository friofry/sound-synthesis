...
uint32_t init(uint32_t free_count, uint32_t row_ptr_len, uint32_t nnz) {
  ...
  cursor += nnz * (uint32_t)sizeof(float);
...
static void compute_spring_acceleration_csr(const float *u, float *out) {
  const uint32_t *row_ptr = u32_ptr(g_offset_row_ptr);
  const uint32_t *col = u32_ptr(g_offset_col);
  const float *coeff = f32_ptr(g_offset_coeff);
  const float *diag = f32_ptr(g_offset_diag);

  for (uint32_t i = 0; i < g_free_count; i += 1) {
    float acc = diag[i] * u[i];
...
void euler_step(float dt, float attenuation, float square_attenuation) {
  ...
  float *spring = f32_ptr(g_offset_spring);
  ...
  for (; i < n4; i += 4) {
    ...
    float acc1 = spring[i + 1] - attenuation * v[i + 1] - square_attenuation * abs_float(v[i + 1]) * v[i + 1];
    float acc2 = spring[i + 2] - attenuation * v[i + 2] - square_attenuation * abs_float(v[i + 2]) * v[i + 2];
    float acc3 = spring[i + 3] - attenuation * v[i + 3] - square_attenuation * abs_float(v[i + 3]) * v[i + 3];
